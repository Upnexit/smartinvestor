...
-- Revoke default PUBLIC execute on all admin_* and sensitive functions; grant to service_role only.
DO $$
DECLARE r record;
BEGIN
  FOR r IN
    ...
    WHERE n.nspname = 'public'
      AND p.proname IN (
        ...
        'admin_upsert_distributor','expire_user_packages'
      )
  LOOP
...
-- has_role and distributor_stats: allow signed-in users only (needed by RLS policies + user UI).
DO $$
DECLARE r record;
BEGIN
  FOR r IN
    SELECT n.nspname, p.proname, pg_get_function_identity_arguments(p.oid) AS args
    FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname IN ('has_role','distributor_stats')
  LOOP

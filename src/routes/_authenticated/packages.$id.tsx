  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      <header className={cn(
        ...
        isVip ? "ring-amber-300" : "ring-emerald-300",
      )}>
        {/* Full-quality image background */}
        {p.image_url ? (
          <img
            src={p.image_url}
            alt={p.name}
        ...
        {/* Readability overlay: dark bottom, tinted top */}
        <div className={cn(
          "absolute inset-0",
          isVip
            ? "bg-gradient-to-t from-rose-950/85 via-orange-900/50 to-amber-900/30"
            : "bg-gradient-to-t from-emerald-950/85 via-teal-900/45 to-cyan-900/25",
        )} />
        <div className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-white/10 blur-2xl" />

        {/* Content on top */}
        <div className="relative p-6 sm:p-8 min-h-[280px] sm:min-h-[340px] flex flex-col justify-end">
          {isVip && (
            <span className="self-start inline-flex items-center gap-1 rounded-full bg-white/20 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider backdrop-blur ring-1 ring-white/30">
              <Star className="h-3 w-3 fill-white" /> Featured
            </span>
          )}
          <h1 className="bn-display mt-3 text-3xl sm:text-4xl drop-shadow-lg">{p.name}</h1>
          {p.description && <p className="mt-2 max-w-xl text-sm text-white/95 drop-shadow">{p.description}</p>}
          <div className="mt-4 flex items-end gap-2">
            <span className="bn-display text-5xl sm:text-6xl drop-shadow-lg">৳{bn(Number(p.price))}</span>
